...
describe("API Integración Extra (GET/:id, PUT, DELETE)", () => {
  ...
  it("Restaurantes: GET /:id (ok) y 404", async () => {
    ...
    const ok = await request(app).get(`/v2/restaurantes/${id}`);
    expect(ok.status).toBe(200);
    expect(ok.body.id).toBe(id);

    const nf = await request(app).get(`/v2/restaurantes/999999`);
  ...
  it("Restaurantes: DELETE /:id (ok) y 404", async () => {
    ...
    const del = await request(app).delete(`/v2/restaurantes/${id}`);
    expect(del.status).toBe(204);

    const nf = await request(app).delete(`/v2/restaurantes/${id}`);
    expect(nf.status).toBe(404);
  });
    ...
    const r = await request(app).get("/v2/restaurantes");
  ...
  it("Menus: GET /:id (ok) y 404", async () => {
    ...
    const ok = await request(app).get(`/v2/v2/menus/${menuId}`);
    expect(ok.status).toBe(200);
    expect(ok.body.id).toBe(menuId);

    const nf = await request(app).get(`/v2/v2/menus/999999`);
    expect(nf.status).toBe(404);
  });
  ...
  it("Menus: DELETE /:id (ok) y 404", async () => {
    ...
    const del = await request(app).delete(`/v2/v2/menus/${menuId}`);
    expect(del.status).toBe(204);

    const nf = await request(app).delete(`/v2/v2/menus/${menuId}`);
    expect(nf.status).toBe(404);
  });
  ...
  it("Reservas: GET /:id (ok) y 404", async () => {
    ...
    const ok = await request(app).get(`/v2/reservas/${reservaId}`);
    expect(ok.status).toBe(200);
    expect(ok.body.id).toBe(reservaId);

    const nf = await request(app).get(`/v2/reservas/999999`);
    expect(nf.status).toBe(404);
  });
  ...
  it("Reservas: DELETE /:id (ok) y 404", async () => {
    ...
    const del = await request(app).delete(`/v2/reservas/${reservaId}`);
    expect(del.status).toBe(204);

    const nf = await request(app).delete(`/v2/reservas/${reservaId}`);
    expect(nf.status).toBe(404);
  });
